...
def main():
    ...
    if(validConnection == True):
        ...
        #just decided to reuse loop without resetting it
        while(loop == False):
            ...
            databaseCursor.execute(query)
            #just easier to chop off the string for a awnser, just easier this way
            ...
            if(mode == '1'):
                ...
                if __name__ == '__main__':
                    logging.debug("query failure")
                    raise queryFailure("query has failed to execute")
